...
public class ArrayString {
    ...
    public static String stringReverse(String string) {
        ...
        for(int i = string.length() - 1; i >= 0; i--) {
            if(Character.isWhitespace(string.charAt(i))) {
                stringBuilder.append(string, i + 1, string.length() - stringBuilder.length()).append(" ");
            }
        }

        stringBuilder.append(string, 0, string.length() - stringBuilder.length());
     ...
     * Algorithm is keep track of index of zero, and whenever you encounter non-zero number swap it with "index of zero" element
    ...
    public int[] moveZeroes(int[] nums) {
        ...
        for(int i=0; i<nums.length; i++){
            if(nums[i]==0){
                if(indexOfZero==-1 || nums[indexOfZero]!=0){
                    indexOfZero=i;
                }
            }else{
                if(indexOfZero!=-1){
    ...
    //appends subtractive notation (4 - IV, 9 - IX, 40 - XL, 900 - CM etc) in char array
    public int sub_digit(char num1, char num2, int i, char[] c) {
        c[i++] = num1;
        c[i++] = num2;
        return i;
    }
    // appends given character in array and return next available index in char array
     ...
     * // create a DP lookup table
		boolean[][] lookup = new boolean[str.length + 1][pattern.length + 1]; main method will pass this
     * Loop through string
     * 		if indexed character in string matches with same indexed pattern character increment index for both string and pattern
     * 		if pattarn[m] == *,
     */

    /* Better solution for wildcard pattern matching
        backtracking, Time comple - O(min(s, p)) for best case and average case - O (s log P)  - s (length(string)), p (length(pattern))
	...
	*/
     ...
     * for >arr.length/2 following answer is correct. for max occurred have max variable maitained and compare char with it
     */
    ...
    public List<List<String>> groupAnagrams(String[] strs) {
        ...
        for(String str: strs){
            ...
            // above sorting can be written as char[] arr = Arrays.sort(str.toCharArray())
     ...
     * initially leftIndex = 0 and rightIndex = str.length;
     */

    public int findMinInsertions(char str[], int leftIndex, int rightIndex){
        ...
        if (leftIndex == rightIndex-1)
            return (str[leftIndex]==str[rightIndex] ? 0:1);

        //find if first and last character are same. based on that we can decide which decide how to calculate min
        return (str[leftIndex]==str[rightIndex] ? findMinInsertions(str, leftIndex+1, rightIndex-1) :
                Math.min(findMinInsertions(str, leftIndex, rightIndex-1), findMinInsertions(str, leftIndex+1, rightIndex))+1);
    }
    ...
    public boolean isReachable(ArrayList<Integer> list, int target) {
        ...
        ArrayList<Integer> results = getResults (list, left, right);
    ...
    public ArrayList<Integer> getResults(ArrayList<Integer> list, int left, int right){

        ArrayList<Integer> results = new ArrayList<>();

        if (left>right){
            return results;
            //if there is only one element left, return that as result
        } else if (left == right) {
    ...
    public int minimumTotal(ArrayList<ArrayList<Integer>> triangle) {
        ...
        // put all last row element in total array
        for (int i = 0; i < triangle.get(l).size(); i++) {
            total[i] = triangle.get(l).get(i);
        }

        // iterate from second last row of triangle
        for (int i = triangle.size() - 2; i >= 0; i--) {
            // add all elements of current row into total array
    ...
    public int minJumpsToReachEnd(int arr[], int leftIndex, int rightIndex)
    {
        ...
        // reachable points.
        int min = Integer.MAX_VALUE;
        for (int i = leftIndex+1; i <= rightIndex && i <= leftIndex + arr[leftIndex]; i++)
        {
            int jumps = minJumpsToReachEnd(arr, i, rightIndex);
    ...
    public void printFrequencySortedArray(int[] array){
        ...
        /* this is what happens behind scene in java 8
    ...
    // another way to find fist missing positive
    ...
    *   validate IP address
    ...
    public List<String> fullJustify(String[] words, int maxWidth) {
        ...
        int count=0; // variable to store total length of all words in words array
        int last=0; // index of last word in last row
    ...
    int countDecodingDP(char []digits, int n)
    {
        ...
        for (int i = 2; i <= n; i++)
        {
            ...
            // smaller than 7, then last two digits form a valid character - [A-Z, Z being 26th ]
    ...
    */
    ...
    public void function(List<List<String>> tickets) {
        List<String> result = new ArrayList<>();
        HashMap<String, TreeSet<String>> map = new HashMap<>();
        for (List<String> ticket : tickets) {
            map.put(ticket.get(0), map.getOrDefault(ticket.get(0), new TreeSet<>()).add(ticket.get(1)));
        }
        ...
        while (!queue.isEmpty()) {
            ...
            queue.offer(map.get(key).pollFirst());
        }
    }
}

/*
1. maitains a max-heap and a min-heap
2. add current number and balance the heaps
3. starting from i == k - 1:
  3.1 get median
  3.2 remove the first element from either the max-heap or min-heap
we maintain the heaps so that min-heap.size() <= max-heap.size() + 1 (but we can also max-heap.size() <= min-heap.size() + 1),
 */
